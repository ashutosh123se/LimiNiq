...
import { PricingSectionClient } from "../pricing/PricingSectionClient";

export async function PricingSection({ showFAQ = false }: { showFAQ?: boolean }) {
  let plans: any[] = [];
  
  try {
    // Fetch active plans from the database, sorted by order
    plans = await prisma.pricingPlan.findMany({
      where: { active: true },
      orderBy: { sortOrder: 'asc' }
    });
  } catch (error) {
    console.warn("Database connection failed or Prisma Client not generated. Falling back to default plans.");
    // Fallback plans if DB is not connected yet
    plans = [
      // WEB DEVELOPMENT
      {
        id: "wd-2",
  ...
  // Pass down to the interactive client component
  return <PricingSectionClient plans={plans} showFAQ={showFAQ} />;
}
